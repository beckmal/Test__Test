...
const COMPARE_DATABASE_PATH = "/mnt/c/Syncthing/MuHa - Bilder/MuHa.xlsx"

"""
    get_database_path() -> String

Returns the path to MuHa.xlsx database, using fallback if primary not found.
"""
function get_database_path()
    if isfile(COMPARE_DATABASE_PATH)
        return COMPARE_DATABASE_PATH
    else
        # Fallback to local directory
        fallback = joinpath(@__DIR__, "MuHa.xlsx")
        @warn "Primary database not found, using fallback: $fallback"
        return fallback
    end
end
...
function create_compare_figure(sets, input_type; max_images_per_row::Int=6, test_mode::Bool=false)
    ...
    function build_patient_images!(patient_id::Int)
        ...
        for (col, entry) in enumerate(entries[1:num_images])
            ...
            # Create observable for this image
            local img_obs = Bas3GLMakie.GLMakie.Observable(get_image_by_index(entry.image_index))
        ...
        Bas3GLMakie.GLMakie.rowsize!(images_grid, 5, Bas3GLMakie.GLMakie.Fixed(40))   # Save button
    end
